...
// 读取方式是同步的
let result = require('./4.home_work.js')
console.log(result)
...
// 带有sync就是同步读取
let file = fs.readFileSync('./4.home_work.js') // 一般情况下能立即拿到返回值的是同步(有返回值不一定是同步)
let exists = fs.existsSync('./4.home_work.js') // 文件是否存在
console.log(exists, file)

// 3) path模块, 专门用来解析路径的: resolve, join, dirname, extname...等
const path = require('path')
console.log(path.join(__dirname, '4.home_work.js'))
// resolve 解析出一个绝对路径, 如果只传入一个名字, 用 process.cwd()解析出路径
console.log(path.resolve(__dirname, '4.home_work.js'))
